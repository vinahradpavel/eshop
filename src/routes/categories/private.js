...
const { categoriesPost, categoriesDelete, categoriesUpdate } = require('../../validators/categories');
...
router.put('/:id', roleAccess({ roles: [ADMIN] }), celebrate(categoriesUpdate), async (req, res, next) => {
  try {
    const { id } = req.params;
